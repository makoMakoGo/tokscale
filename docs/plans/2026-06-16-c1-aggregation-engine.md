...
> **Status note (PR #59):** This document is the original implementation plan, not the final source of truth for the landed interface. The implementation moved TUI view types into `tokscale_core::usage_views`, so `AggregatedViews` now carries `Option<UsageData>` directly and `ViewSet::TUI` is driven by `AggregationEngine`. After old report folds were deleted, model/monthly/hourly tests are engine contract and entrypoint-consistency tests rather than independent old-vs-new parity; graph/session/time-metrics still compare the engine with the primitive `aggregator`/`sessionize` composition.
