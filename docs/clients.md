...
In the table, `$XDG_DATA_HOME` defaults to `~/.local/share` and
`$XDG_STATE_HOME` defaults to `~/.local/state`. Client-specific home variables
use the parenthesized default when unset. `**` means recursive discovery under
the stated root.
...
| --- | --- | --- | --- |
| `opencode` | OpenCode | `$XDG_DATA_HOME/opencode/opencode.db` and `opencode-<channel>.db`; direct files from `scanner.opencodeDbPaths` | Reads SQLite with committed WAL state, joins messages to sessions, validates assistant token payloads, and deduplicates across databases. |
...
| `codex` | Codex CLI | `$CODEX_HOME/sessions/**/*.jsonl`; `$CODEX_HOME/archived_sessions/**/*.jsonl` (`CODEX_HOME=~/.codex`) | Reads provider-written interactive and exec session events with append-aware parsing and stable cross-file deduplication. |
| `gemini` | Gemini CLI | `$GEMINI_CLI_HOME/tmp/<named-project>/chats/session-*.json` and `session-*.jsonl` (`GEMINI_CLI_HOME=~/.gemini`) | Requires `.project_root` in each named project directory and uses it as workspace identity. |
| `amp` | Amp | `$XDG_DATA_HOME/amp/threads/**/T-*.json` | Reads token-bearing events from each thread usage ledger. |
...
| `kimi` | Kimi | `$KIMI_CODE_HOME/sessions/**/agents/*/wire.jsonl` (`KIMI_CODE_HOME=~/.kimi-code`) | Reads ordered per-agent `usage.record` events and reconciles each model alias with its applicable `llm.request` or current config entry. |
...
| `kilo` | Kilo | `$XDG_DATA_HOME/kilo/kilo.db` | Reads the Kilo SQLite store with committed WAL state for all Kilo frontends using that data environment. |
| `hermes` | Hermes | `$HERMES_HOME/state.db` (`HERMES_HOME=~/.hermes`) | Reads token records from Hermes state and derives cost from Tokscale pricing. |
| `copilot` | Copilot | `~/.copilot/otel/*.jsonl`; the file selected by `COPILOT_OTEL_FILE_EXPORTER_PATH` | Reads Copilot OTEL file-export records and workspace metadata. |
| `goose` | Goose | `$GOOSE_PATH_ROOT/data/sessions/sessions.db` when set; otherwise `$XDG_DATA_HOME/goose/sessions/sessions.db` or `~/Library/Application Support/goose/sessions/sessions.db` | Reads the first available Goose SQLite session store with committed WAL state. |
| `codebuff` | Codebuff | `$CODEBUFF_DATA_DIR/projects/**/chat-messages.json` when set; otherwise `~/.config/{manicode,manicode-dev,manicode-staging}/projects/**/chat-messages.json` | Reads Codebuff conversation token records. `CODEBUFF_DATA_DIR` selects its projects tree exclusively. |
...
| `antigravity` | Antigravity | `$GEMINI_CLI_HOME/antigravity-cli/conversations/*.db` (`GEMINI_CLI_HOME=~/.gemini`) | Reads AGY CLI SQLite with committed WAL state and decodes the current protobuf token-accounting fields. |
| `zed` | Zed Agent | `$XDG_DATA_HOME/zed/threads/threads.db`; platform Zed application-data equivalent | Reads hosted Zed assistant usage from SQLite with committed WAL state and excludes external-agent records by ownership fields. |
...
| `cline` | Cline | `$CLINE_SESSION_DATA_DIR/**/*.messages.json`; otherwise `$CLINE_DATA_DIR/sessions`, `$CLINE_DIR/data/sessions`, or `~/.cline/data/sessions` | Reads the SDK v1 messages envelope and optional root manifest workspace metadata. The environment variables form an exclusive precedence chain. |
...
| `grok` | Grok | `$GROK_HOME/sessions/**/updates.jsonl` (`GROK_HOME=~/.grok`) with optional `summary.json` and `events.jsonl` siblings | Reads positive total-token deltas and optional session metadata. |
...
For one invocation, `TOKSCALE_EXTRA_DIRS` uses the same client-to-root mapping:

```bash
TOKSCALE_EXTRA_DIRS='codex:/srv/imports/codex/sessions,gemini:/srv/imports/gemini/tmp' \
  tokscale models --no-spinner
```

Generic extra roots are accepted for the table IDs except `opencode`, `kilo`,
and `goose`. OpenCode accepts explicit database files instead:
...
before parsing. An explicit `--home` resolves built-in roots beneath the
supplied home and disables ambient client-home and one-invocation root
variables.
...
- Linux and FreeBSD:
  `$XDG_STATE_HOME/{warp-terminal,warp-terminal-preview,warp-terminal-dev,warp-terminal-local,warp-oss}/warp.sqlite`;
...
- Windows:
  `%LOCALAPPDATA%\warp\{Warp,WarpPreview,WarpDev,WarpLocal,WarpOss}\data\warp.sqlite`.
