...
// fixes should bump the relevant DiscoveredInput decoder revision instead.
...
macro_rules! define_decoder_ids {
    ($($variant:ident => ($stable_name:literal, $plain_kind:literal)),+ $(,)?) => {
...
fn initialize_input_record_shards(cache_dir: &Path) -> std::io::Result<()> {
...
impl CachedInputEntry {
    ...
    pub(crate) fn new(
        ...
        Self::new_with_revision(path, 1, fingerprint, records, codex_incremental)
    }
...
#[derive(Serialize)]
struct BorrowedCachedUsageRecord<'a> {
    model_id: &'a str,
    provider_id: &'a str,
    session_id: &'a str,
    is_main_session: bool,
    ...
    timestamp: i64,
    tokens: &'a TokenBreakdown,
    message_count: i32,
    agent: Option<&'a str>,
    agent_instance: Option<&'a str>,
    dedup_key: Option<u64>,
    is_turn_start: bool,
}
...
struct BorrowedCachedRecords<'a>(&'a [UsageRecord]);
...
struct CachedShardBody {
    records: Vec<CachedUsageRecord>,
}

#[derive(Serialize)]
struct BorrowedCachedShardBody<'a> {
    records: BorrowedCachedRecords<'a>,
}
...
impl InputRecordShardStore {
    ...
    pub(crate) fn write_records(
        ...
        write_shard_borrowed(&self.cache_dir, &plan, records).map_err(|source| {
            InputRecordCacheError::io(
    ...
    pub(crate) fn save_if_dirty(&mut self) -> Result<(), InputRecordCacheError> {
        ...
        for key in &self.deleted_paths {
            ...
            match fs::remove_file(&shard_path) {
                ...
                Err(source) if source.kind() == std::io::ErrorKind::NotFound => {}
                Err(source) => {
                    return Err(InputRecordCacheError::io(
    ...
    fn protect(&self, key: &CachedInputKey) -> bool {
        self.protected_paths
            .lock()
            .expect("input-record cache protected-path lock poisoned")
            .insert(key.clone())
    }
...
fn header_from_plan(plan: &CacheWritePlan, record_count: usize) -> CachedShardHeader {
    CachedShardHeader {
        decoder_version: plan.decoder_version,
...
#[cfg(unix)]
fn sync_removed_shard_parent(shard_path: &Path) -> Result<(), InputRecordCacheError> {
    let parent = shard_path.parent().ok_or_else(|| {
        InputRecordCacheError::io(
            "locate removed input-record cache shard parent",
            shard_path,
            std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                "input-record cache shard path has no parent",
            ),
        )
    })?;
    File::open(parent)
        ...
        .map_err(|source| {
            InputRecordCacheError::io(
                "sync removed input-record cache shard directory",
                parent,
                source,
            )
        })
...
#[cfg(not(unix))]
fn sync_removed_shard_parent(_shard_path: &Path) -> Result<(), InputRecordCacheError> {
    Ok(())
}

fn shard_paths_for_prune(shards_dir: &Path) -> Result<Vec<PathBuf>, InputRecordCachePruneError> {
...
fn read_shard_header_for_prune(
    ...
    file.read_exact(&mut len_bytes).map_err(|source| {
        InputRecordCachePruneError::current_format_io(
            "read input-record cache shard header length",
            path,
            format_version,
            source,
        )
    ...
    let header_len = u64::from_le_bytes(len_bytes);
    if header_len == 0 || header_len > MAX_SHARD_HEADER_BYTES {
        return Err(InputRecordCachePruneError::InvalidHeaderLength {
            path: path.to_path_buf(),
            format_version,
            actual: header_len,
        });
    ...
    file.read_exact(&mut header_bytes).map_err(|source| {
        InputRecordCachePruneError::current_format_io(
            "read input-record cache shard header",
            path,
            format_version,
            source,
        )
    ...
    bincode::options()
        ...
        .deserialize(&header_bytes)
        .map(Some)
        .map_err(|source| InputRecordCachePruneError::Decode {
            path: path.to_path_buf(),
            format_version,
            source,
        })
...
mod tests {
    ...
    fn restore_cache_env(
        ...
        restore_env_var("TOKENX_CONFIG_DIR", prev.3);
    }
