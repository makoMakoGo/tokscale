...
use super::{
    ClientIntegration, InputPipelineError, InputUnit, ParseContext, ParsedUnit, UnifiedMessageSink,
};
...
impl<'a> FoldContext<'a> {
    ...
    pub(crate) fn reparse_one(&self, unit: InputUnit) -> Result<ParsedUnit, InputPipelineError> {
        let mut reparsed = self.integration.parse_checked(
            vec![unit],
            &ParseContext {
...
impl<'a> BoundMessageSink<'a> {
    pub(crate) fn new(
        integration: &dyn ClientIntegration,
        downstream: &'a mut dyn UnifiedMessageSink,
    ) -> Self {
        Self {
