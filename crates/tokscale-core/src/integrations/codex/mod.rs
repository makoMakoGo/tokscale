...
#[cfg(test)]
use crate::integrations::DecoderRoute;
use crate::integrations::{
    ...
    InputPlanningError, InputUnit, ParseContext, ParsedBatchInput, ParsedUnit, SourceSpec,
    UnitMessagePayload,
};
use crate::records::ParsedMessage;
use crate::{message_cache, pricing, records};

pub(crate) struct Integration;
const SOURCE: SourceSpec = SourceSpec::home(".codex/sessions", "*.jsonl");

#[derive(Debug)]
pub(crate) struct CodexAppendInput {
    path: PathBuf,
    read_plan: message_cache::CacheReadPlan,
    decoder_version: message_cache::DecoderVersion,
...
impl ClientIntegration for Integration {
    ...
    fn discover_checked(
        ...
        let units = adapter_discover::input_units_from_paths_preserving_order(
            ...
            DecoderSpec::codex(crate::integrations::CODEX_EXEC_IDENTITY_REVISION),
        )?;
        Ok(units)
    }
...
fn plan_exact_codex_cache_hit(
    ...
    let read_plan =
        message_cache::CacheReadPlan::new(&unit.path, unit.decoder.version(), cached.fingerprint);
...
fn load_or_parse_codex_unit(
    ...
    let input_snapshot = unit.take_input_snapshot().map_err(|source| {
        records::error::SessionParseError::at_path(&path, "read Codex input snapshot", source)
    })?;
...
mod tests {
    ...
    fn codex_binding() -> &'static dyn crate::integrations::ClientIntegration {
        crate::integrations::integration_for(ClientId::Codex)
    }

    fn codex_fold_context<'a>(
        cache: &'a mut message_cache::InputMessageCache,
    ...
    fn fold_codex_into(
        ...
        let mut sink = BoundMessageSink::new(codex_binding(), messages);
        INTEGRATION.fold(parsed, ctx, &mut sink)
    }
    ...
    fn fold_codex_batches_into(
        ...
        let mut sink = BoundMessageSink::new(codex_binding(), messages);
        INTEGRATION.fold_batches(batches, ctx, &mut sink)
    }
        ...
        for unit in units {
                ...
                .expect("Codex cache planning must succeed")
            {
                ...
                CacheHitPlan::Miss(miss) => {
                    parsed.extend(INTEGRATION.parse_checked(vec![miss], &ParseContext { pricing }))
                }
            }
        }
        parsed
    ...
    fn assert_cached_raw_messages_match_parser(cache_home: &Path, path: &Path) {
        let expected =
            decode::parse_codex_file_incremental(path, 0, decode::CodexParseState::default())
                .unwrap();
        let decoder_version = message_cache::DecoderVersion::new(
    ...
    fn codex_adapter_discovers_sessions_archived_and_extra_roots() {
        ...
        assert!(units
            .iter()
            .all(|unit| matches!(unit.decoder.route(), DecoderRoute::Codex)));
    }
    ...
    fn codex_adapter_reports_malformed_record_without_losing_input() {
        ...
        let parsed =
            INTEGRATION.parse_checked(vec![codex_unit(&path)], &ParseContext { pricing: None });

        assert_eq!(parsed.len(), 1);
        let health = &parsed[0].health;
        assert_eq!(codex_binding().client(), ClientId::Codex);
    ...
    fn codex_adapter_cache_hit_matches_fresh_parse() {
        ...
        let parsed = vec![expect_codex_hit(
            INTEGRATION.plan_cache_hit(prepared_codex_unit(&path), &cache),
    ...
    fn codex_truncated_body_is_repaired_and_second_read_is_a_true_warm_hit() {
        ...
        let planned = expect_codex_hit(
            INTEGRATION.plan_cache_hit(prepared_codex_unit(&path), &repair_cache),
        ...
        let warm = expect_codex_hit(
            INTEGRATION.plan_cache_hit(prepared_codex_unit(&path), &warm_cache),
    ...
    fn codex_unknown_header_is_preserved_when_reparse_is_not_cacheable() {
        ...
        parse_and_fold(vec![codex_unit(&path)], &mut seed_cache);
        let shard_path =
            message_cache::shard_path_for_test(cache_home.path(), &path, decoder_version);
    ...
    fn codex_warm_cache_rejects_same_size_same_mtime_atomic_replacement() {
        ...
        let miss = expect_codex_miss(
            INTEGRATION.plan_cache_hit(prepared, &cache),
