...
use crate::integrations::{
    BoundMessageSink, CacheHitPlan, ClientIntegration, DecoderSpec, DiscoveryContext,
    FingerprintPolicy, FoldContext, InputDiscoveryError, InputPipelineError, InputPlanningError,
    InputUnit, ParseContext, ParsedBatchInput, ParsedUnit, SourceSpec,
};
...
const SOURCE: SourceSpec = SourceSpec::home(".omp/agent/sessions", "*.jsonl");
...
impl ClientIntegration for Integration {
    ...
    fn discover_checked(
        ...
        let units = adapter_discover::discover_default_scanned_units(
            ...
            DecoderSpec::plain(DecoderId::Omp, OMP_RECORD_REJECTION_REVISION),
        )?
        .into_iter()
        .map(|unit| {
    ...
    fn fold(
        ...
        for unit in parsed {
            if matches!(
                unit.messages,
                crate::integrations::UnitMessagePayload::CacheHit(_)
            ) {
                hit_units.push(unit);
            } else {
...
fn omp_parent_health_unit(path: PathBuf, cacheable: bool) -> InputUnit {
    let decoder = DecoderSpec::plain(DecoderId::OmpParentHealth, OMP_PARENT_HEALTH_REVISION);
    if cacheable {
        InputUnit::plain_file(path, decoder)
    } else {
        InputUnit::no_message_cache(path, decoder)
    }
}
...
fn plan_parent_health_cache(
    ...
    for candidate in candidates {
        ...
        match adapter_cache::plan_cache_hit(unit, input_cache) {
            ...
            let mut parsed = match health.status {
                ...
                crate::input_health::InputStatus::Partial { failure } => {
                    let mut parsed = ParsedUnit::healthy(
                        miss.unit,
                        crate::integrations::UnitMessagePayload::Fresh(Vec::new()),
                        None,
                        true,
                    );
...
fn fold_omp_cache_hits(
    ...
    ctx: &mut FoldContext<'_>,
    mut sink: Option<&mut BoundMessageSink<'_>>,
        ...
        match adapter_cache::resolve_messages(messages, ctx) {
            Ok(messages) => {
                ...
                if let Some(sink) = sink.as_deref_mut() {
                    adapter_cache::emit_messages(messages, sink);
                }
            }
            Err(failure) => {
...
fn omp_parent_health_units(
    ...
    parent_index
        ...
        .map(|health| {
            let unit = InputUnit::no_message_cache(
                health.path,
                DecoderSpec::plain(DecoderId::OmpParentHealth, OMP_PARENT_HEALTH_REVISION),
            );
            let mut parsed = ParsedUnit::healthy(
                unit,
                crate::integrations::UnitMessagePayload::Fresh(Vec::new()),
...
{"type":"message","id":"child_001","parentId":null,"timestamp":"2026-01-01T00:00:02.000Z","message":{"role":"assistant","model":"gpt-5.5","provider":"openai","usage":{"input":20,"output":10,"cacheRead":0,"cacheWrite":0,"totalTokens":30}}}"#;
    ...
    fn binding() -> &'static dyn crate::integrations::ClientIntegration {
    ...
    fn unit(path: PathBuf) -> InputUnit {
        InputUnit::plain_file(path, decoder())
    }

    fn unit_with_parent(path: PathBuf, parent_path: PathBuf) -> InputUnit {
    ...
    fn fold_parsed(
        ...
        let mut sink = crate::integrations::BoundMessageSink::new(binding, &mut messages);
        INTEGRATION
            .fold(
                parsed,
    ...
    fn fold_batches_with_omp_adapter(
        units: Vec<InputUnit>,
        cache: &mut message_cache::InputMessageCache,
    ) -> (Vec<crate::UnifiedMessage>, crate::input_health::DataHealth) {
        let binding = binding();
        let mut messages = Vec::new();
        let mut sink = crate::integrations::BoundMessageSink::new(binding, &mut messages);
    ...
    fn seed_omp_disk_cache(cache_dir: &Path, unit: &InputUnit, session_id: &str) {
    ...
    fn unchanged_parent_hits_warm_child_cache_and_parent_change_refreshes_agent() {
        ...
        let mut cache = message_cache::InputMessageCache::default();
        let parsed =
            INTEGRATION.parse_checked(vec![child_unit.clone()], &ParseContext { pricing: None });
        ...
        let warm = match INTEGRATION.plan_cache_hit(child_unit, &cache).unwrap() {
        ...
        let refreshed_child_unit = INTEGRATION
            .discover_checked(&ctx)
            .unwrap()
            .into_iter()
            .find(|unit| unit.path == child_path)
        ...
        let parsed = INTEGRATION.parse_checked(vec![miss], &ParseContext { pricing: None });
    ...
    fn cold_children_do_not_reread_their_shared_parent_for_cache_hashes() {
        ...
        assert_eq!(
            message_cache::get_input_read_stats(&parent_path).hash_passes,
            0,
    ...
    fn omp_batched_fold_preserves_global_parent_index_and_hit_first_order() {
        ...
        let child_unit = unit(child_path);
        let cached_unit = unit(cached_path.clone()).prepare_snapshot().unwrap();
        let parent_unit = unit(parent_path);
        let mut cache = message_cache::InputMessageCache::default();
        cache.insert(message_cache::CachedInputEntry::new_with_version(
            &cached_path,
            decoder_version,
    ...
    fn omp_batched_fold_keeps_child_usage_when_parent_has_a_malformed_record() {
        ...
        let mut messages = Vec::new();
        let mut sink = crate::integrations::BoundMessageSink::new(binding, &mut messages);
        let mut batches = crate::integrations::ParsedBatchInput::new(binding, vec![unit]);
    ...
    fn warm_child_only_extra_root_preserves_shared_parent_health() {
        ...
        let cold_units = INTEGRATION.discover_checked(&scan_ctx).unwrap();
        assert_eq!(
            cold_units
                .iter()
        ...
        let warm_units = INTEGRATION.discover_checked(&scan_ctx).unwrap();
        ...
        decode::reset_omp_parent_scan_count(&parent_path);
        let changed_units = INTEGRATION.discover_checked(&scan_ctx).unwrap();
    ...
    fn damaged_parent_health_body_is_rebuilt_once() {
        ...
        let mut cold_cache = message_cache::InputMessageCache::with_cache_dir(cache_dir.path());
        let cold_units = INTEGRATION.discover_checked(&scan_ctx).unwrap();
        ...
        decode::reset_omp_parent_scan_count(&parent_path);
        let mut repair_cache = message_cache::InputMessageCache::with_cache_dir(cache_dir.path());
        let repair_units = INTEGRATION.discover_checked(&scan_ctx).unwrap();
    ...
    fn parent_health_header_faults_reparse_the_input() {
        ...
        let future_cache =
            message_cache::InputMessageCache::with_cache_dir(future_cache_dir.path());
    ...
    fn unavailable_parent_health_preserves_old_shard_without_serving_it() {
        ...
        decode::reset_omp_parent_scan_count(&parent_path);
        let unavailable_units = INTEGRATION.discover_checked(&scan_ctx).unwrap();
        ...
        decode::reset_omp_parent_scan_count(&parent_path);
        let retry_units = INTEGRATION.discover_checked(&scan_ctx).unwrap();
    ...
    fn omp_dependency_cache_restores_messages_and_rejection_health() {
        ...
        let mut cache = message_cache::InputMessageCache::default();
        cache.insert(entry);
        let hit = match INTEGRATION.plan_cache_hit(unit, &cache).unwrap() {
            CacheHitPlan::Hit(hit) => hit,
        ...
        INTEGRATION.fold(vec![hit], &mut ctx, &mut sink).unwrap();
