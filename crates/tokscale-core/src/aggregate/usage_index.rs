//! Canonical usage indexing: one fold over `UnifiedMessage`s producing
//! [`crate::usage_views::UsageView`] (models/agents/daily/hourly/graph/streaks).
//! `AggregationEngine` owns this accumulator when `ViewSet::USAGE` is requested;
//! canonical generation loading drives that engine instead of the CLI carrying its own
//! fold (#37). The fold is group-independent: messages land in canonical
//! finest-granularity buckets, and [`UsageIndex::project`] re-folds them into any
//! grouping's `UsageView` in memory (issue #161).
...
use crate::usage_views::{
    ...
    HourlyModelInfo, HourlyUsage, PeriodKind, PeriodUsage, UsageGraphData, UsageModelEntry,
    UsageTokenBreakdown, UsageView,
};
use crate::{
    aggregate::keys::{
    ...
    ClientId, GroupBy, UnifiedMessage,
};
...
/// canonical usage index owned by `AggregationEngine` when `ViewSet::USAGE` is
...
/// [`UsageView`] and may be called repeatedly with different groupings —
/// switching the grouping no longer rescans, reparses, or reprices local
/// clients (issue #161).
...
impl UsageIndex {
    ...
    /// Materialize one grouping's [`UsageView`] from the canonical fold
    ...
    pub fn project(&self, group_by: &GroupBy) -> UsageView {
        self.project_selected(group_by, None)
    }

    /// Materialize a usage view for a session-local subset of the clients that
    ...
    fn project_selected(
        ...
        let (current_streak, longest_streak) = calculate_streaks_for_today(&daily, today);
...
mod tests {
    ...
    use crate::records::UnifiedMessage;
    ...
    fn structured_session_and_agent_instance_identities_do_not_alias_delimiters() {
        ...
        let mut model_acc = UsageIndex::new();
        ...
        let mut agent_acc = UsageIndex::new();
    ...
    fn reprojection_accumulator() -> UsageIndex {
        let mut acc = UsageIndex::new();
    ...
    fn assert_usage_data_eq(left: &UsageView, right: &UsageView) {
    ...
    fn totals_use_canonical_full_and_client_scoped_folds() {
        ...
        let claude_and_codex = acc.project_for_clients(
            &GroupBy::Model,
        ...
        let qwen = acc.project_for_clients(&GroupBy::Model, &HashSet::from([ClientId::Qwen]));
    ...
    fn canonical_accumulator_round_trip_preserves_full_and_subset_totals() {
        let acc = reprojection_accumulator();
        let encoded = serde_json::to_vec(&acc).expect("serialize canonical canonical usage index");
        let restored: UsageIndex =
            serde_json::from_slice(&encoded).expect("deserialize canonical canonical usage index");
    ...
    fn client_projection_matches_a_fresh_fold_of_only_the_selected_clients() {
        ...
        for selected in [
            ...
            let mut expected = UsageIndex::new();
    ...
    fn reprojection_preserves_group_independent_views() {
        ...
        for group_by in [
            ...
            let daily_rollup = |data: &UsageView| {
                data.daily
                    .iter()
                    .map(|day| {
                        (
            ...
            let hourly_rollup = |data: &UsageView| {
                data.hourly
                    .iter()
                    .map(|hour| {
                        (
