...
fn run_tui_with_input(input: &[u8]) -> (ExitStatus, Vec<u8>) {
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_tokscale"))
        .args(["tui", "--no-refresh"])
        .env("HOME", home.path())
        .env("TOKSCALE_CONFIG_DIR", home.path().join("tokscale-config"))
...
fn q_exits_tui_successfully_after_restoring_terminal() {
    let (status, _) = run_tui_with_input(b"q");
    assert_eq!(status.code(), Some(0));
}

#[test]
fn ctrl_c_exits_tui_with_130_after_restoring_terminal() {
    let (status, _) = run_tui_with_input(b"\x03");
