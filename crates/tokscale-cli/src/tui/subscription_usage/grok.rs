...
pub fn has_credentials() -> bool {
    match auth_path() {
        ...
        // Let the provider fetch surface the missing-home fault instead of
        // reducing it to the generic "credentials not found" diagnostic.
        Err(_) => true,
    }
}
...
fn usage_output(credentials: &Credentials, response: BillingConfigResponse) -> Result<UsageOutput> {
...
mod tests {
    ...
    fn normalizes_current_credit_and_subscription_response() -> Result<()> {
        ...
        let output = usage_output(&credentials(), response)?;

        assert_eq!(output.provider, "Grok");
        assert_eq!(output.display_name(), "Grok (First Last)");
