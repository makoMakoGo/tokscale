...
fn parse_home_arg(raw: &str) -> Result<String, String> {
...
fn parse_pricing_provider(raw: &str) -> Result<String, String> {
    match raw.to_lowercase().as_str() {
        "custom" | "litellm" | "openrouter" | "models.dev" => Ok(raw.to_string()),
        _ => Err(format!(
            "invalid provider: {raw}. Valid providers: custom, litellm, openrouter, models.dev"
        )),
    }
}
...
enum Commands {
    ...
    Pricing {
        ...
        #[arg(
            long,
            value_parser = parse_pricing_provider,
