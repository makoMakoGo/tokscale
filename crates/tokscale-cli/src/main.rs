...
use commands::shared::auto_sync_cursor_before_tui;
...
fn execute(plan: ExecutionPlan) -> Result<()> {
    match plan {
        ExecutionPlan::Tui(plan) => {
            auto_sync_cursor_before_tui(&plan.source.home, &plan.source.clients)?;
            tui::run(
                plan.theme.as_deref(),
                plan.refresh,
                plan.no_refresh,
        ...
        ExecutionPlan::Codex(subcommand) => run_codex_command(subcommand),
        ExecutionPlan::Cursor(subcommand) => run_cursor_command(subcommand),
