...
| <img width="48px" src="https://github.com/CommandCodeAI.png" alt="Command Code" /> | [Command Code](https://commandcode.ai/) | `~/.commandcode/projects/**/*.jsonl`（估算使用量；跳过 checkpoint 日志） | ✅ 支持 |
...
| OMP | OMP | `~/.omp/agent/sessions/` | ✅ 支持 |
| <img width="48px" src=".github/assets/client-kimi.png" alt="Kimi" /> | [Kimi](https://github.com/MoonshotAI/kimi-code) | `$KIMI_CODE_HOME/sessions/`（备用：`~/.kimi-code/sessions/`） | ✅ 支持 |
...
| <img width="48px" src=".github/assets/client-antigravity.png" alt="Antigravity" /> | [Google Antigravity](https://antigravity.google/) / [Antigravity CLI](https://github.com/google-antigravity/antigravity-cli) | IDE 通过 `tokscale antigravity sync` 缓存到 `~/.config/tokscale/antigravity-cache/sessions/*.jsonl`；CLI 数据库位于 `$GEMINI_CLI_HOME/antigravity-cli/conversations/*.db`（回退：`~/.gemini/antigravity-cli/conversations/*.db`） | ✅ 支持 |
...
| <img width="48px" src="https://github.com/kirodotdev.png" alt="Kiro" /> | Kiro | `~/.kiro/sessions/cli/*.json`（+ `*.jsonl`）、`~/.local/share/kiro-cli/data.sqlite3` 和 Kiro IDE `globalStorage/kiro.kiroagent` 快照 | ✅ 支持 |
...
- **交互式 TUI 模式** - 由 Ratatui 驱动的精美终端 UI（默认模式）
  - 8 个核心交互式视图：概览、模型、每月、每周、每日、每时、统计、代理（可选的 Usage 视图通过 `usageTabEnabled` 启用）
  - 键盘和鼠标导航
  - 9 种颜色主题的 GitHub 风格贡献图
  - 实时筛选和排序
  - 零闪烁渲染
- **多平台支持** - 跟踪 OpenCode、Claude Code、Codex CLI、Copilot CLI、Cursor IDE、Gemini CLI、Amp、Codebuff、Command Code、Droid、OpenClaw、Hermes Agent、Pi、OMP、Kimi、Qwen CLI、Roo Code、Kilo、Mux、Kilo CLI、Goose、Antigravity、Zed、ZCode、Kiro 和 Trae 的使用情况
...
- **8 个核心视图**：概览（图表 + 热门模型）、模型、每月、每周、每日、每时、统计（贡献图）、代理。可选的 Usage（订阅额度）默认隐藏，可在 `settings.json` 中通过 `usageTabEnabled` 启用，参见[配置](#配置)
- **键盘导航**：
  - `←/→/Tab/BackTab`：切换视图
  - `↑/↓` 或 `Home/End`：导航列表
  - `Enter`：打开周期/每日详情（Monthly、Weekly、Daily 标签）/ 选择图表单元格（Stats 标签）
  ...
  - `y`：复制选中行到剪贴板
  - `p`：循环 9 种颜色主题
  - `r`：刷新本地报告；`Shift+R` 切换本地报告自动刷新；`+`/`-` 调整间隔；`u` 仅在 Usage 标签刷新订阅额度
  - `e`：导出为 JSON
  - `q` 或 `Ctrl+C`：退出
- **鼠标支持**：点击标签、按钮和筛选器
...
可用值：`opencode`、`claude`、`codex`、`copilot`、`gemini`、`cursor`、`amp`、`codebuff`、`droid`、`openclaw`、`hermes`、`pi`、`omp`、`kimi`、`qwen`、`roocode`、`kilocode`、`kilo`、`mux`、`goose`、`antigravity`、`zed`、`kiro`、`trae`。
...
> **价格说明**：Trae API 记录包含 token buckets，也可能包含 `dollar_float`；本地报告会忽略 vendor cost 字段，并通过统一 pricing service 从 token 推导成本。
...
在 TUI 中，将 `usageTabEnabled` 设为 `true`，并在 `usageProviders` 中列出允许远程请求的提供商后，切换到 **Usage** 标签即可查看订阅数据。每个 TUI 会话首次进入 Usage 标签时最多自动请求一次；之后只能在 Usage 标签按 `u` 显式刷新订阅额度。`r` 始终只刷新本地报告，`R` 只控制本地报告自动刷新；tokscale 不会后台轮询订阅额度。

空 `usageProviders` 表示缓存展示模式：Usage 标签可以显示已缓存的订阅数据，但不会发起远程请求。`tokscale usage` 命令不同：显式执行该命令本身就代表用户授权，因此 CLI 仍会尝试所有具备可用凭据的 provider。
...
| Provider | 认证方式 | 指标 | 设置 |
|----------|----------|------|------|
| **Claude** | OAuth（凭据文件或 macOS Keychain） | 5 小时 Session、Weekly、Opus 额度 | 运行 `claude` 登录 |
| **Codex** | OAuth（`~/.config/codex/auth.json` 或 `~/.codex/auth.json`） | Session、Weekly 额度 | 运行 `codex` 登录 |
| **Z.ai GLM Coding Plan** | GLM Coding Plan API key（`TOKSCALE_USAGE_ZAI_CODING_PLAN_API_KEY`） | Token limits、Web Searches | 使用 GLM Coding Plan key，不是普通 Z.ai API key |
| **Amp** | API key（`~/.local/share/amp/secrets.json`） | Free tier balance、Credits | 运行 `amp` 登录 |
| **GitHub Copilot** | GitHub token（keychain 或 `~/.config/gh/hosts.yml`） | Premium interactions、Chat 额度 | 运行 `gh auth login` |
| **Kimi Code** | Kimi Code API key（`TOKSCALE_USAGE_KIMI_CODING_PLAN_API_KEY`）或 Kimi Code OAuth（`$KIMI_CODE_HOME/credentials/kimi-code.json`） | Session、Weekly 额度 | 使用 Kimi Code Console API key，或运行 `kimi` 登录 |
| **MiniMax Token Plan CN** | 国内 Token Plan 订阅 key（`TOKSCALE_USAGE_MINIMAX_TOKEN_PLAN_CN_KEY`） | 5 小时与周 Token Plan 窗口 | 使用国内控制台的 Token Plan key |
| **MiniMax Token Plan Global** | 国际 Token Plan 订阅 key（`TOKSCALE_USAGE_MINIMAX_TOKEN_PLAN_GLOBAL_KEY`） | 5 小时与周 Token Plan 窗口 | 使用国际控制台的 Token Plan key |

TUI `usageProviders` 的 canonical ID：`claude`、`codex`、`zai`、`amp`、`copilot`、`grok`、`kimi`、`minimax-token-plan-cn`、`minimax-token-plan-global`、`warp`。

`zai` 指 Z.ai/智谱 GLM Coding Plan 额度，不是普通 API 余额；GLM Coding Plan 不拆国内/国际 provider ID。MiniMax Token Plan 拆国内与国际，两个 key 不通用。`ZAI_API_KEY`、`GLM_API_KEY`、`KIMI_API_KEY`、`MINIMAX_API_KEY`、`MINIMAX_API_TOKEN` 这类通用变量会被有意忽略。
...
| `defaultClients` | string[] | `[]` | 未传递 `--client/-c` 选项时应用的客户端筛选。接受与 `--client` 相同的 ID（例如 `["opencode", "claude", "zed"]`）。未知 ID 会被静默丢弃。命令行选项会完全覆盖此列表 — 不会合并。 |
...
| `usageProviders` | string[] | `[]` | TUI Usage 标签允许请求的订阅 provider 显式白名单。空数组表示缓存展示模式：只显示缓存，不发送远程订阅请求。 |

#### 启用 Usage 标签

Usage 视图显示订阅额度与重置时间，适合在本地 Token 使用统计之外查看提供商账号级额度。它默认隐藏，因为额度查询依赖提供商接口，且不是本地用量分析的主流程。

要启用远程订阅请求，请在 `~/.config/tokscale/settings.json` 中同时设置 `usageTabEnabled` 和非空 `usageProviders`：
...
重启后，Usage 标签会出现在 TUI 标签栏中。也可以直接运行 `tokscale usage`、`tokscale usage --json` 或 `tokscale usage --light` 查看订阅额度；这些 CLI 命令会自动尝试所有具备凭据的 provider。
...
| `TOKSCALE_USAGE_ZAI_CODING_PLAN_API_KEY` | unset | Z.ai/智谱 GLM Coding Plan key，用于订阅额度查询。 |
| `TOKSCALE_USAGE_KIMI_CODING_PLAN_API_KEY` | unset | Kimi Code Console API key，用于 Kimi Code 会员额度查询。 |
| `TOKSCALE_USAGE_MINIMAX_TOKEN_PLAN_CN_KEY` | unset | MiniMax 国内 Token Plan 订阅 key。 |
| `TOKSCALE_USAGE_MINIMAX_TOKEN_PLAN_GLOBAL_KEY` | unset | MiniMax 国际 Token Plan 订阅 key。 |
...
- **年份筛选**：在年份之间导航
- **来源筛选**：按平台筛选（OpenCode、Claude、Codex、Copilot、Cursor、Gemini、Amp、Codebuff、Command Code、Droid、OpenClaw、Hermes Agent、Pi、OMP、Kimi、Qwen、Roo Code、Kilo、Mux、Kilo CLI、Goose、Antigravity、Zed、ZCode、Kiro、Trae）
...
| OMP | `~/.omp/` | `%USERPROFILE%\.omp\` | 所有平台使用相同路径 |
| Kimi | `~/.kimi-code/` | `%USERPROFILE%\.kimi-code\` | 可通过 `KIMI_CODE_HOME` 环境变量配置 |
...
| Command Code | `~/.commandcode/projects/**/*.jsonl` | `%USERPROFILE%\.commandcode\projects\**\*.jsonl` | 基于 transcript 的估算使用量；跳过 checkpoint 日志 |
...
| Antigravity | IDE 缓存：`~/.config/tokscale/antigravity-cache/sessions/`；CLI 数据库：`$GEMINI_CLI_HOME/antigravity-cli/conversations/*.db`（回退：`~/.gemini/antigravity-cli/conversations/*.db`） | CLI 数据库：`%USERPROFILE%\.gemini\antigravity-cli\conversations\*.db` | IDE 同步缓存和 CLI 数据库都归入同一个 `antigravity` 来源 |
...
| ZCode | `~/.zcode/projects/**/*.jsonl` | `%USERPROFILE%\.zcode\projects\**\*.jsonl` | 解析 Z.ai ADE JSONL 会话 transcript |
...
位置：`~/.config/tokscale/antigravity-cache/sessions/*.jsonl`（通过本地 Antigravity 语言服务器 RPC 同步）以及 `$GEMINI_CLI_HOME/antigravity-cli/conversations/*.db`（回退：`~/.gemini/antigravity-cli/conversations/*.db`）

Antigravity IDE 数据不会被根命令自动获取。请在启用了 Antigravity 的编辑器打开时运行 `tokscale antigravity sync` 来刷新本地缓存。Antigravity CLI 数据会直接从本地会话 SQLite 数据库读取，不需要 sync；两者都会归入同一个 `antigravity` client。

### Command Code

位置：`~/.commandcode/projects/**/*.jsonl`

Command Code transcript 不在本地保存权威 token 使用量。Tokscale 会从 transcript 内容估算 assistant turn：input 是每次 assistant 响应前的累计上下文，output 是 assistant 响应内容。Command Code 默认不包含在 submit 数据里；本地报告可用 `--client commandcode` 显式包含。

### Trae

位置：`~/.config/tokscale/trae-cache/sessions/*.json`（通过官方使用量 API 同步）

Trae 数据不会被根命令自动获取。先运行一次 `tokscale trae login`，然后在生成报告前运行 `tokscale trae sync`。Tokscale 会将包含 token buckets 的同步 API dump 解析为会话级记录；如果 Trae 返回 `dollar_float`，本地报告会忽略它，并通过统一 pricing service 从 token 推导成本。
...
Hermes 将会话级使用量存储在 SQLite `sessions` 表中。Tokscale 导入 `model` 存在且 token usage 为正的行，使用 `started_at` 作为时间戳，保留 `message_count`，忽略 `actual_cost_usd` 和 `estimated_cost_usd`，并通过统一 pricing service 从 token 推导报告成本。
...
位置：`~/.omp/agent/sessions/<encoded-cwd>/*.jsonl`

OMP 使用与 Pi 相同的 JSONL 消息格式，并以 client id `omp`、显示标签 `OMP` 统计。

### Kimi

位置：`$KIMI_CODE_HOME/sessions/{WORKDIR_KEY}/{SESSION_ID}/agents/{AGENT_ID}/wire.jsonl`（备用：`~/.kimi-code/sessions/{WORKDIR_KEY}/{SESSION_ID}/agents/{AGENT_ID}/wire.jsonl`）

包含 `usage.record` 条目的 wire.jsonl 格式：
```json
