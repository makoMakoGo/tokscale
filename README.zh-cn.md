...
| <img width="48px" src="https://github.com/XiaomiMiMo.png" alt="MiMo Code" /> | [MiMo Code](https://github.com/XiaomiMiMo/MiMo-Code) | `~/.local/share/micode/mimocode.db` | ✅ 支持 |
...
| <img width="48px" src=".github/assets/client-antigravity.png" alt="Antigravity CLI" /> | [Antigravity CLI](https://github.com/google-antigravity/antigravity-cli) | `$GEMINI_CLI_HOME/antigravity-cli/conversations/*.db`（回退：`~/.gemini/antigravity-cli/conversations/*.db`） | ✅ 支持 |
...
- **交互式 TUI 模式** - 由 Ratatui 驱动的精美终端 UI（默认模式）
  ...
  - 实时筛选和排序
  - 零闪烁渲染
- **多平台支持** - 跟踪 OpenCode、Claude Code、Codex CLI、Copilot CLI、Cursor IDE、Gemini CLI、Amp、Codebuff、MiMo Code、Command Code、Droid、OpenClaw、Hermes Agent、Pi、OMP、Kimi、Qwen CLI、Roo Code、Kilo、Mux、Kilo CLI、Crush、Goose、Antigravity、Antigravity CLI、Zed、Kiro、Trae 和 Gajae-Code 的使用情况
...
可用值：`opencode`、`claude`、`codex`、`copilot`、`gemini`、`cursor`、`amp`、`codebuff`、`droid`、`openclaw`、`hermes`、`pi`、`omp`、`kimi`、`qwen`、`roocode`、`kilocode`、`kilo`、`mux`、`crush`、`goose`、`antigravity`、`zed`、`kiro`、`trae`、`gjc`。
...
- **年份筛选**：在年份之间导航
- **来源筛选**：按平台筛选（OpenCode、Claude、Codex、Copilot、Cursor、Gemini、Amp、Codebuff、MiMo Code、Command Code、Droid、OpenClaw、Hermes Agent、Pi、OMP、Kimi、Qwen、Roo Code、Kilo、Mux、Kilo CLI、Crush、Goose、Antigravity、Antigravity CLI、Zed、Kiro、Trae、Gajae-Code）
...
| MiMo Code | `~/.local/share/micode/mimocode.db` | `%USERPROFILE%\.local\share\micode\mimocode.db` | 带 token 和成本的本地 SQLite 数据库 |
...
| Antigravity CLI | `$GEMINI_CLI_HOME/antigravity-cli/conversations/*.db`（回退：`~/.gemini/antigravity-cli/conversations/*.db`） | `%USERPROFILE%\.gemini\antigravity-cli\conversations\*.db` | 本地会话 SQLite 数据库 |
...
位置：`$GEMINI_CLI_HOME/antigravity-cli/conversations/*.db`（回退：`~/.gemini/antigravity-cli/conversations/*.db`）

Antigravity CLI 数据会直接从本地会话 SQLite 数据库读取。它与上面的 Antigravity IDE RPC 缓存是两个来源，不需要运行 `tokscale antigravity sync`。
...
位置：`~/.local/share/micode/mimocode.db`

MiMo Code 数据来自本地 SQLite 数据库。assistant 行包含 token bucket 和可选成本，tokscale 会保留这些内嵌值。
