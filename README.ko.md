...
| OMP | OMP | `~/.omp/agent/sessions/` | ✅ 지원 |
| <img width="48px" src=".github/assets/client-kimi.png" alt="Kimi" /> | [Kimi](https://github.com/MoonshotAI/kimi-code) | `$KIMI_CODE_HOME/sessions/` (fallback: `~/.kimi-code/sessions/`) | ✅ 지원 |
...
- **인터랙티브 TUI 모드** - Ratatui 기반의 터미널 UI (기본 모드)
  - 8개 인터랙티브 뷰: 개요, 모델, 월별, 주별, 일별, 시간별, 통계, 에이전트
  - 키보드 및 마우스 지원
  - 9가지 테마의 GitHub 스타일 기여 그래프
  - 실시간 필터링 및 정렬
  - 깜빡임 없는 렌더링
- **멀티 플랫폼 지원** - OpenCode, Claude Code, Codex CLI, Copilot CLI, Cursor IDE, Gemini CLI, Amp, Codebuff, Droid, OpenClaw, Hermes Agent, Pi, OMP, Kimi, Qwen CLI, Roo Code, Kilo, Mux, Kilo CLI, Crush, Goose, Antigravity, Zed, ZCode, Kiro, Trae, Gajae-Code 사용량 통합 추적
...
가능한 값: `opencode`, `claude`, `codex`, `copilot`, `gemini`, `cursor`, `amp`, `codebuff`, `droid`, `openclaw`, `hermes`, `pi`, `omp`, `kimi`, `qwen`, `roocode`, `kilocode`, `kilo`, `mux`, `crush`, `goose`, `antigravity`, `zed`, `kiro`, `trae`, `gjc`.
...
- **연도 필터링**: 연도 간 탐색
- **소스 필터링**: 플랫폼별 필터 (OpenCode, Claude, Codex, Copilot, Cursor, Gemini, Amp, Codebuff, Droid, OpenClaw, Hermes Agent, Pi, OMP, Kimi, Qwen, Roo Code, Kilo, Mux, Kilo CLI, Crush, Goose, Antigravity, Zed, ZCode, Kiro, Trae, Gajae-Code)
...
| OMP | `~/.omp/` | `%USERPROFILE%\.omp\` | 모든 플랫폼에서 동일한 경로 |
| Kimi | `~/.kimi-code/` | `%USERPROFILE%\.kimi-code\` | `KIMI_CODE_HOME` 환경 변수로 설정 가능 |
...
| ZCode | `~/.zcode/projects/**/*.jsonl` | `%USERPROFILE%\.zcode\projects\**\*.jsonl` | Z.ai ADE JSONL 세션 transcript 파싱 |
...
위치: `~/.omp/agent/sessions/<encoded-cwd>/*.jsonl`

OMP는 Pi와 동일한 JSONL 메시지 형식을 사용하며 client id `omp`, 표시 라벨 `OMP`로 집계됩니다.

### Kimi

위치: `$KIMI_CODE_HOME/sessions/{WORKDIR_KEY}/{SESSION_ID}/agents/{AGENT_ID}/wire.jsonl` (fallback: `~/.kimi-code/sessions/{WORKDIR_KEY}/{SESSION_ID}/agents/{AGENT_ID}/wire.jsonl`)

`usage.record` 항목을 포함하는 wire.jsonl 형식:
```json
